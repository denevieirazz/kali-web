...
function Get-CodeSigningCertificateV22 {
    ...
    if ($found.Count -ne 1) {
        throw "Expected exactly one certificate with thumbprint $WantedThumbprint; found $($found.Count)."
    }
    ...
    $eku = @($certificate.EnhancedKeyUsageList | ForEach-Object { [string]$_.ObjectId.Value })
...
$privateMaterial = @(
    Get-ChildItem -LiteralPath $stage -Recurse -File |
        Where-Object { $_.Extension.ToLowerInvariant() -in @('.pfx', '.p12', '.key', '.pem') }
)
...
$certificate = Get-CodeSigningCertificateV22 -WantedThumbprint $thumbprint
$signable = @(
    Get-ChildItem -LiteralPath $stage -Recurse -File |
        Where-Object { $_.Extension.ToLowerInvariant() -in @('.exe', '.dll', '.ps1', '.psm1') } |
        Sort-Object FullName
)
if ($signable.Count -eq 0) { throw 'No Authenticode-signable files exist in the staged release.' }

$signatureRecords = New-Object System.Collections.Generic.List[object]
foreach ($file in $signable) {
    ...
    if ($signed.Status -ne 'Valid') {
        throw "Authenticode signing failed for $($file.Name): $($signed.Status) $($signed.StatusMessage)"
    }
    ...
    $signatureRecords.Add([pscustomobject]@{
...
# Authenticode modifies PE/script bytes. Refresh package integrity metadata only
# after every signature has verified so the final hashes describe signed bytes.
...
foreach ($name in $binaries) {
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Signed payload missing: $name" }
    $record = @($manifest.files | Where-Object { $_.name -eq $name })
    if ($record.Count -ne 1) { throw "Manifest must contain exactly one record for $name." }
    $record[0].size = [Int64](Get-Item -LiteralPath $path).Length
    $record[0].sha256 = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant()
}
...
[ordered]@{
    ...
    all_valid = $true
    signed_file_count = $signatureRecords.Count
...
foreach ($line in Get-Content -LiteralPath $sumPath) {
    ...
    $name = $Matches[2]
    $actualHash = (Get-FileHash -LiteralPath (Join-Path $stage $name) -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($actualHash -ne $expectedHash) { throw "Final signed SHA256 mismatch: $name" }
}

if (Test-Path -LiteralPath $OutputZip) { Remove-Item -LiteralPath $OutputZip -Force }
Compress-Archive -Path (Join-Path $stage '*') -DestinationPath $OutputZip -CompressionLevel Optimal
if (-not (Test-Path -LiteralPath $OutputZip -PathType Leaf) -or (Get-Item -LiteralPath $OutputZip).Length -le 0) {
...
Write-Host "[CloudOS V22] AUTHENTICODE_OK files=$($signatureRecords.Count) signer=$thumbprint"
