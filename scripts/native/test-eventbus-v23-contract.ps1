...
Assert-Contains $eventCpp 'method == "start"' 'Event transport must start only after Dart installs its channel handler.'
Assert-Contains $eventCpp 'method == "stop"' 'Event transport must expose deterministic stop semantics.'
Assert-Contains $eventCpp 'method == "status"' 'Event transport must expose read-only status diagnostics.'
...
Assert-Contains $flutterWindow 'InitializeChannel' 'Flutter window must initialize the event channel with the engine messenger.'
Assert-Contains $flutterWindow 'CloudOSBrokerEventClientV23::Instance().Stop()' 'Flutter window teardown must stop the event transport before engine teardown.'
Assert-Contains $flutterWindow 'kDispatchMessage' 'Flutter window must drain native events on its platform message loop.'

Assert-Contains $dartBridge "cloudos/native/events/v23" 'Dart bridge is not bound to the dedicated event channel.'
Assert-Contains $dartBridge "invokeMethod<Object?>('start')" 'Dart must start native events only after installing the handler.'
...
Assert-Contains $runtime "event.name == 'job.progress'" 'Runtime service must explicitly suppress job.progress notification spam.'
Assert-Contains $runtime 'nativeDroppedEventCount' 'Runtime service lost native drop telemetry.'
Assert-Contains $taskbar 'RuntimeEventService.unreadCount' 'Taskbar no longer uses real unread notification state.'
