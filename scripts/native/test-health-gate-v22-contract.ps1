...
foreach ($path in @($modulePath, $suitePath)) {
...
if ($errors.Count -ne 0) {
    throw "Health Gate V22 module has PowerShell parse errors: $($errors.Message -join '; ')"
}
...
Write-Host '[PASS] Health Gate V22: one shared runtime-stop, Authenticode evidence and Supervisor readiness/heartbeat authority for install/update/repair.'
