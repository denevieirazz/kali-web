...
# Broker-only structural contract for the bounded active WSL health probe.
...
foreach ($path in @($probeHeaderPath, $probeSourcePath, $wslHeaderPath, $brokerSourcePath, $systemSourcePath, $projectPath)) {
...
if ($probeHeader -match '\bcommand\b\s*[,;)]' -or $probeHeader -match '\bargv\b\s*[,;)]' -or $probeHeader -match '\bshell_command\b') {
...
if ($probeSource -notmatch 'CreateProcessW' -or $probeSource -notmatch 'WaitForSingleObject' -or $probeSource -notmatch 'TerminateProcess') {
...
if ($probeSource -notmatch 'FindDistribution\(snapshot' -or $probeSource -notmatch 'base_path_present') {
...
if ($brokerSource -notmatch 'wsl_probe_service_v22\.h' -or $brokerSource -notmatch 'method\s*==\s*"wsl\.health\.probe"') {
    throw 'System Broker must expose the typed wsl.health.probe RPC.'
}
if ($brokerSource -notmatch 'field\.first\s*!=\s*"distro"[\s\S]{0,100}field\.first\s*!=\s*"timeoutMs"' -or
    $brokerSource -notmatch "accepts only 'distro' and 'timeoutMs'") {
    throw 'The WSL health RPC must use a strict payload allowlist.'
}
if ($brokerSource -notmatch 'requested_timeout\s*<\s*1000' -or $brokerSource -notmatch 'requested_timeout\s*>\s*15000') {
    throw 'The Broker must reject out-of-contract probe deadlines.'
}
if ($brokerSource -notmatch 'WriteWslProbePayload' -or
    $brokerSource -notmatch '"healthy"' -or $brokerSource -notmatch '"timedOut"' -or
    $brokerSource -notmatch '"markerSeen"' -or $brokerSource -notmatch '"exitCode"' -or
...
if ($brokerSource -notmatch 'event interleaving here could make it consume an event as the reply') {
    throw 'The synchronous V21 probe RPC must preserve response framing before event publication.'
}
if ($systemSource -notmatch '"wsl\.health\.probe"') {
    throw 'System Broker capabilities must advertise wsl.health.probe only once the active RPC exists.'
}
if ($wslHeader -notmatch 'base_path_present') {
    throw 'Active probe preflight depends on passive distro storage evidence.'
}
if ($project -notmatch 'src\\wsl_probe_service_v22\.h' -or $project -notmatch 'src\\wsl_probe_service_v22\.cpp') {
    throw 'WslProbeServiceV22 must be compiled into CloudOS.SystemBroker.'
}
if ($project -match 'event_transport_v23') {
    throw 'WSL Probe V22 must not pull Event Transport V23 into this milestone.'
}

Write-Host '[PASS] CloudOS bounded active WSL health probe Broker contract.'
