...
foreach ($path in @($updatePath, $v13Path)) {
...
foreach ($required in @(
    ...
    'signature_all_valid',
    'Assert-CloudOSRuntimeStoppedForUpdate',
...
foreach ($forbidden in @(
    ...
    if ($update.IndexOf($forbidden, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
        throw "Transactional V22 update introduced forbidden behavior: $forbidden"
    }
}

# Parse-only validation catches malformed continuations/quotes before runtime.
$tokens = $null
$errors = $null
...
if ($errors.Count -ne 0) {
    throw "Transactional V22 update script has PowerShell parse errors: $($errors.Message -join '; ')"
}

Write-Host '[PASS] Transactional Update V22 contract: immutable V13 staging + Authenticode evidence + runtime-stop preflight + active supervisor health gate + rollback.'
