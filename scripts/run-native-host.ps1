...
$UpdatedHost = Join-Path $Root 'desktop\publish-next\CloudOS.Host.exe'
$PublishedHost = if (Test-Path -LiteralPath $UpdatedHost) {
  $UpdatedHost
} else {
  Join-Path $Root 'desktop\publish\CloudOS.Host.exe'
}
