...
if (-not $EvidenceDirectory) {
    $EvidenceDirectory = Join-Path $root 'test-results\dependency-worktree'
}
...
try {
    ...
    foreach ($relative in @('node_modules','frontend/node_modules','backend/node_modules')) {
        if (Test-Path -LiteralPath (Join-Path $temp $relative)) {
            throw "WORKTREE_NOT_CLEAN_NODE_MODULES_PRESENT:$relative"
        }
    }
    ...
    if ($before.missing -notcontains 'directory:node_modules') {
        throw 'WORKTREE_PREFLIGHT_DID_NOT_DETECT_MISSING_NODE_MODULES'
    }

    $result = Ensure-CloudOSNodeDependencies -Root $temp -EvidenceDirectory $EvidenceDirectory -AllowInstall
    if (-not $result.complete -or -not $result.installPerformed) {
        throw 'WORKTREE_BOOTSTRAP_DID_NOT_INSTALL'
    }
    ...
    foreach ($relative in @('node_modules','frontend/node_modules','backend/node_modules')) {
        ...
        if (($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "NODE_MODULES_JUNCTION_FORBIDDEN:$relative"
        }
    }
    ...
    if ($status.Count -gt 0) {
        throw "WORKTREE_TRACKED_FILES_CHANGED:$($status -join ',')"
    }
    ...
    @{
        ...
        head = $head
        worktree = $temp
        nodeModulesInitiallyAbsent = $true
        rootInstallPerformed = $true
        noNodeModulesJunction = $true
    ...
    if ($added -and (Test-Path -LiteralPath $temp)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            $cleanupError = "WORKTREE_REMOVE_FAILED:$LASTEXITCODE:$temp"
        }
