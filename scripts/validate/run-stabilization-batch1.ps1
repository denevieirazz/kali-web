...
try {
    ...
    $pwsh = [string]$pwshResolution.path
    ...
    Write-JsonFile (Join-Path $runDirectory 'git.json') ([ordered]@{
        branch=$branch; head=$head; expectedBase=$expectedBase; mergeBase=$mergeBase; timestamp=(Get-Date).ToUniversalTime().ToString('o')
    })
    Write-JsonFile (Join-Path $runDirectory 'environment.json') ([ordered]@{
        os=[Environment]::OSVersion.VersionString; machine=$env:COMPUTERNAME; powershell=$PSVersionTable.PSVersion.ToString(); powershellPath=$pwsh;
        node=(& $node --version 2>$null | Select-Object -First 1); nodePath=$node; npm=(& $npm --version 2>$null | Select-Object -First 1); npmPath=$npm;
        npxPath=$npx; dotnet=(& $dotnet --version 2>$null | Select-Object -First 1); dotnetPath=$dotnet;
        isolatedDataDirectory=$isolatedDataDirectory; databasePath=$env:DATABASE_PATH; realDatabaseUsed=$false
    })
    Write-JsonFile (Join-Path $runDirectory 'manifest.json') ([ordered]@{
        schemaVersion=1; batch='stabilization-batch-1'; branch=$branch; head=$head; base=$expectedBase; executionId=$executionId;
        startedAt=(Get-Date).ToUniversalTime().ToString('o'); status='running'; physicalEntryPoint='Validar CloudOS.cmd'; resultDirectory=$runDirectory
    })
    ...
    function Save-ProcessSnapshot {
        ...
        $items=Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Select-Object ProcessId,ParentProcessId,Name,ExecutablePath,CreationDate
    ...
    function Save-WslSnapshot {
        ...
        $lines=@('### wsl.exe --list --verbose',(& wsl.exe --list --verbose 2>&1),'','### wsl.exe --status',(& wsl.exe --status 2>&1))
    ...
    function Get-ComponentLogTail {
        ...
        $candidates=switch($Component){'backend'{@('backend.stderr.log','backend.stdout.log')};'frontend'{@('frontend.stderr.log','frontend.stdout.log')};'host'{@('host.stderr.log','host.log')};default{@('launcher.log')}}
        foreach($name in $candidates){$path=Join-Path ([string]$Session.logDirectory) $name;if(Test-Path -LiteralPath $path){$tail=(Get-Content -LiteralPath $path -Tail 30 -ErrorAction SilentlyContinue)-join ' | ';if($tail){return [ordered]@{path=$path;tail=$tail}}}}
        return [ordered]@{path=[string]$Session.logDirectory;tail=''}
    }

    function Assert-SessionProcessesAlive {
        param([Parameter(Mandatory)]$Session,[Parameter(Mandatory)][string]$Mode)
        ...
        foreach($record in @($manifest.processes)){
            ...
            if(-not $process){$log=Get-ComponentLogTail -Session $Session -Component ([string]$record.component);throw "SESSION_PROCESS_EXITED:$Mode:$($record.component):pid=$($record.pid):log=$($log.path):error=$($log.tail)"}
            try{$actualStart=$process.StartTime.ToUniversalTime().ToString('o')}catch[System.InvalidOperationException]{$log=Get-ComponentLogTail -Session $Session -Component ([string]$record.component);throw "SESSION_PROCESS_EXITED:$Mode:$($record.component):pid=$($record.pid):log=$($log.path):error=$($log.tail)"}
            if($actualStart -ne [string]$record.startedAt){throw "SESSION_PROCESS_OWNERSHIP_LOST:$Mode:$($record.component):pid=$($record.pid)"}
        }
    }

    function Assert-NoRecordedProcessesRemain {
        param([Parameter(Mandatory)]$Session)
        foreach($record in @($Session.processes)){$process=Get-Process -Id ([int]$record.pid) -ErrorAction SilentlyContinue;if(-not $process){continue};try{$sameProcess=$process.StartTime.ToUniversalTime().ToString('o') -eq [string]$record.startedAt;if($sameProcess){throw "ORPHAN_SESSION_PROCESS:$($record.component):$($record.pid)"}}catch[System.InvalidOperationException]{continue}}
    }
    ...
    function Invoke-LauncherSmoke {
        ...
        if(Test-Path -LiteralPath $currentState){try{$current=Get-Content -LiteralPath $currentState -Raw|ConvertFrom-Json;if($current.status -in @('starting','running')){foreach($record in @($current.processes)){$process=Get-Process -Id ([int]$record.pid) -ErrorAction SilentlyContinue;if(-not $process){continue};try{if($process.StartTime.ToUniversalTime().ToString('o') -eq [string]$record.startedAt){throw "ACTIVE_CLOUDOS_SESSION_DETECTED:$($current.id)"}}catch[System.InvalidOperationException]{}}}}catch{if($_.Exception.Message -like 'ACTIVE_CLOUDOS_SESSION_DETECTED:*'){throw}}}
        $session=$null;$started=$false
        try{
            Invoke-CapturedCommand "launcher-$($Mode.ToLowerInvariant())-start" $pwsh @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',(Join-Path $root 'scripts\launch\start-cloudos.ps1'),'-Mode',$Mode)
            $started=$true;Start-Sleep -Milliseconds 300
            ...
            if($session.mode -ne $Mode -or $session.status -ne 'running'){throw "SESSION_NOT_RUNNING:${Mode}:$($session.status)"}
            Assert-SessionProcessesAlive -Session $session -Mode $Mode
            $components=@($session.processes|ForEach-Object{$_.component})
            if($Mode -eq 'WebOnly' -and ($components -notcontains 'backend' -or $components -notcontains 'frontend')){throw 'WEBONLY_EXPECTED_COMPONENTS_MISSING'}
            if($Mode -eq 'Full' -and $components -notcontains 'host'){throw 'FULL_HOST_PROCESS_MISSING'}
            $launcherSessions.Add([ordered]@{mode=$Mode;id=$session.id;logDirectory=$session.logDirectory;dataDirectory=$session.dataDirectory;processes=@($session.processes)})
            ...
            if($ManualCheckpoint -and -not $NonInteractive){
                $checkpoint=@('CHECKPOINT MANUAL EXTERNO - pressionar ENTER nao significa aprovacao automatica.','Inspecionar no Full: Browser pelo Menu Iniciar; Terminal abrir/fechar sem afetar outras apps; onboarding/recovery; Files.','A aprovacao fisica/visual pertence ao Gemini Low, usuario e Copilot principal.')
            ...
            if($started){try{Invoke-CapturedCommand "launcher-$($Mode.ToLowerInvariant())-stop" $pwsh @('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',(Join-Path $root 'scripts\launch\stop-cloudos.ps1'))}finally{Start-Sleep -Milliseconds 500;if($session){Assert-NoRecordedProcessesRemain $session}}}
        }
    }
    ...
    foreach($contract in @('scripts/test-native-host-freshness.ps1','scripts/test-wsl-core-foundation-contract.ps1','scripts/test-wsl-core-secure-terminal-contract.ps1','scripts/test-visible-terminal-wsl-core-contract.ps1')){$name=[IO.Path]::GetFileNameWithoutExtension($contract);Invoke-CapturedCommand $name $pwsh @('-NoLogo','-NoProfile','-File',(Join-Path $root $contract))}
    ...
    Write-JsonFile (Join-Path $runDirectory 'summary.json') ([ordered]@{
        schemaVersion=1;status='passed';branch=$branch;head=$head;base=$expectedBase;commandCount=$commandResults.Count;launcherModes=@($launcherSessions|ForEach-Object{$_.mode});
        dependencyStrategy='root-workspaces-single-install';dependencyInstallPerformed=$dependencyResult.installPerformed;isolatedDataDirectory=$isolatedDataDirectory;realDatabaseUsed=$false;
        wslMutationCommandsExecuted=$false;wslReadOnlySnapshotStable=($beforeWsl -eq $afterWsl);physicalAndVisualApproval='external-pending';finishedAt=(Get-Date).ToUniversalTime().ToString('o')
    })
    Write-JsonFile (Join-Path $runDirectory 'manifest.json') ([ordered]@{schemaVersion=1;batch='stabilization-batch-1';branch=$branch;head=$head;base=$expectedBase;executionId=$executionId;status='passed';physicalEntryPoint='Validar CloudOS.cmd';resultDirectory=$runDirectory;finishedAt=(Get-Date).ToUniversalTime().ToString('o')})
    ...
    if($runDirectory -and (Test-Path -LiteralPath $runDirectory)){Write-JsonFile (Join-Path $runDirectory 'summary.json') ([ordered]@{schemaVersion=1;status='failed';error=$_.Exception.Message;physicalAndVisualApproval='external-pending';finishedAt=(Get-Date).ToUniversalTime().ToString('o')})}
    throw
}finally{
