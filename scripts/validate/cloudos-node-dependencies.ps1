...
function Get-CloudOSRootLockWorkspaces {
    param(
        [Parameter(Mandatory)][string]$Root,
...
const workspaces = lock && lock.packages && lock.packages[""] && lock.packages[""].workspaces;
if (!Array.isArray(workspaces)) {
  console.error("ROOT_LOCK_WORKSPACES_MISSING");
  process.exit(2);
}
for (const workspace of workspaces) console.log(workspace);
'@

    $output = @(& $NodePath -e $script $lockPath 2>&1)
    $exitCode = $LASTEXITCODE
    if ($exitCode -ne 0) {
        return [ordered]@{ complete = $false; workspaces = @(); error = ($output -join "`n") }
    }

    $workspaces = @($output | ForEach-Object { ([string]$_).Trim() } | Where-Object { $_ })
    $required = @('backend','frontend','shared')
    $missing = @($required | Where-Object { $workspaces -notcontains $_ })
    return [ordered]@{
...
for (const name of ["express","ws","dotenv"]) {
  ...
  while (dir && dir !== path.dirname(dir)) {
    ...
    if (fs.existsSync(candidate)) {
      ...
      if (pkg.name === name) {
        console.log(`${name}|${pkg.version}|${entry}`);
        found = true;
        break;
      }
    }
  ...
  if (!found) throw new Error(`PACKAGE_METADATA_NOT_FOUND:${name}:${entry}`);
}
'@

    $output = @(& $NodePath -e $script $backend 2>&1)
    if ($LASTEXITCODE -ne 0) {
        return [ordered]@{ complete = $false; error = ($output -join "`n"); modules = @{} }
    }
    ...
    foreach ($line in $output) {
        ...
        if ($parts.Count -eq 3) {
            $modules[$parts[0]] = [ordered]@{ version = $parts[1]; path = $parts[2] }
        }
    }
...
function Get-CloudOSNodeDependencyState {
    ...
    $rootLockWorkspaces = @()
    $rootLockError = $null
    if ($locks['package-lock.json'] -and $nodePath) {
        $lockState = Get-CloudOSRootLockWorkspaces -Root $root -NodePath $nodePath
        $rootLockCoherent = [bool]$lockState.complete
        $rootLockWorkspaces = @($lockState.workspaces)
        $rootLockError = $lockState.error
    }
    ...
    return [ordered]@{
        ...
        locks = $locks
        rootLockCoversWorkspaces = $rootLockCoherent
        rootLockWorkspaces = $rootLockWorkspaces
        rootLockError = $rootLockError
        nodeModulesRoot = $nodeModulesRoot
        nodeModulesRootExists = $rootNodeModulesExists
        node = $nodePath
        npm = $npmPath
        localBinaries = [ordered]@{ tsc = $tsc; vite = $vite }
        backendModules = $moduleResolution.modules
        backendModuleResolutionError = $moduleResolution.error
    }
}
...
function Invoke-CloudOSRootNpmCi {
    ...
    Push-Location $root
    try {
        & $npmPath @arguments 1> $stdoutPath 2> $stderrPath
        $exitCode = $LASTEXITCODE
    } finally {
        Pop-Location
    }
    ...
    $record = [ordered]@{
        strategy = 'root-workspaces-single-install'
        command = "$npmPath $($arguments -join ' ')"
        executable = $npmPath
        arguments = $arguments
        ...
        packageLockAuthority = (Join-Path $root 'package-lock.json')
        localWorkspaceLocksVerified = @(
        ...
        stdout = $stdoutPath
        stderr = $stderrPath
        nodeVersion = (& $nodePath --version 2>$null | Select-Object -First 1)
    ...
    if ($exitCode -ne 0) {
        $tail = if (Test-Path -LiteralPath $stderrPath) {
            (Get-Content -LiteralPath $stderrPath -Tail 30 -ErrorAction SilentlyContinue) -join ' | '
        } else { '' }
...
function Get-CloudOSDependencyVersions {
    ...
    foreach ($module in @('express','ws','dotenv')) {
        if ($State.backendModules.Contains($module)) { $versions[$module] = $State.backendModules[$module].version }
    }
    return $versions
}
...
function Ensure-CloudOSNodeDependencies {
    ...
    if (-not $before.complete) {
        if (-not $AllowInstall) {
            throw "DEPENDENCY_PREFLIGHT_INCOMPLETE:$($before.missing -join ','):install-disabled"
        }
    ...
    if (-not $after.complete) {
        throw "DEPENDENCY_BOOTSTRAP_INCOMPLETE:$($after.missing -join ',')"
    }
    ...
    $resolved = [ordered]@{
        node = $after.node
        npm = $after.npm
        tsc = $after.localBinaries.tsc
        vite = $after.localBinaries.vite
