...
$stdout=Join-Path $logs 'backend.stdout.log';$stderr=Join-Path $logs 'backend.stderr.log'
$process=$null
...
try{
    ...
    $info.Environment['PATH']=$system32
    $info.Environment['NODE_ENV']='production'
    $info.Environment['HOST']='127.0.0.1'
    $info.Environment['PORT']='0'
    $info.Environment['CLOUDOS_RUNTIME_DIR']=$runtime
    $info.Environment['CLOUDOS_DATA_DIR']=$data
    $info.Environment['CLOUDOS_LOCAL_ROOT']=$temp
    $info.Environment['CLOUDOS_FRONTEND_DIST']=$frontend
    $info.Environment['CLOUDOS_SUPERVISOR_TOKEN']=$token
    $process=[Diagnostics.Process]::new();$process.StartInfo=$info
    if(-not $process.Start()){throw 'PACKAGED_NODE_BACKEND_START_FAILED'}
    $outTask=$process.StandardOutput.ReadToEndAsync();$errTask=$process.StandardError.ReadToEndAsync()
    $manifestPath=Join-Path $runtime 'backend-port.json'
    $deadline=[DateTime]::UtcNow.AddSeconds(30);$manifest=$null
    ...
    $api="http://127.0.0.1:$([int]$manifest.backendPort)"
    $health=Invoke-RestMethod -Uri "$api/api/health" -Method Get -TimeoutSec 4
    ...
    Set-Content -LiteralPath $stdout -Value ($outTask.GetAwaiter().GetResult()) -Encoding utf8
    Set-Content -LiteralPath $stderr -Value ($errTask.GetAwaiter().GetResult()) -Encoding utf8
    Write-Host "PRODUCTIZATION_PACKAGED_NODE_RUNTIME_OK pid=$($process.Id) globalNode=false"
}finally{
