...
function Invoke-ExactWindowsExe{
    ...
    $info.FileName=$FilePath;$info.WorkingDirectory=$testRoot;$info.UseShellExecute=$false;$info.CreateNoWindow=$true
...
try{
    Invoke-ExactWindowsExe ([string]$result.setup) @('--silent','--installto',$installRoot,'--log',$log) | Out-Null
    ...
    Invoke-ExactWindowsExe $updateExe @('uninstall','--silent') | Out-Null
    Start-Sleep -Milliseconds 500
    if(-not(Test-Path -LiteralPath (Join-Path $dataRoot 'sentinel.txt'))){throw 'UNINSTALL_REMOVED_USER_DATA'}
    Write-Host "PRODUCTIZATION_INSTALLER_OK installRoot=$installRoot dataPreserved=true exactArgv=true"
}finally{
    if(Test-Path -LiteralPath (Join-Path $installRoot 'Update.exe')){try{Invoke-ExactWindowsExe (Join-Path $installRoot 'Update.exe') @('uninstall','--silent') -AllowFailure | Out-Null}catch{}}
