...
try{
    ...
    $wslSmokeOutput=& (Join-Path $PSScriptRoot 'test-wsl-core-package-smoke.ps1') 2>&1|Out-String;$wslSmokeOutput|Set-Content -LiteralPath (Join-Path $resultDir 'wsl-core-smoke.txt') -Encoding utf8;Write-Host $wslSmokeOutput.Trim()
    ...
    Invoke-CloudOSExternal ([string]$package.setup) @('--silent','--installto',$installRoot,'--log',(Join-Path $resultDir 'install.log')) $resultDir | Out-Null;Record 1 'instalar pacote experimental' 'PASS' "installRoot=$installRoot"
...
"@.TrimStart()|Set-Content -LiteralPath $launcher -Encoding ascii
    ...
    Invoke-CloudOSExternal (Join-Path $installRoot 'Update.exe') @('uninstall','--silent') $installRoot | Out-Null;if(-not(Test-Path -LiteralPath (Join-Path $validationData 'preservation-sentinel.txt'))){throw 'PHYSICAL_UNINSTALL_DID_NOT_PRESERVE_DATA'};Record 15 'desinstalar preservando dados' 'PASS'
    Invoke-CloudOSExternal ([string]$package.setup) @('--silent','--installto',$installRoot) $resultDir | Out-Null;Record 16 'reinstalar' 'PASS';if(-not(Test-Path -LiteralPath (Join-Path $validationData 'preservation-sentinel.txt'))){throw 'PHYSICAL_REINSTALL_DATA_MISSING'};Record 17 'confirmar dados preservados' 'PASS'
    Invoke-CloudOSExternal (Join-Path $installRoot 'Update.exe') @('uninstall','--silent') $installRoot | Out-Null;& (Join-Path $PSScriptRoot 'remove-cloudos-data.ps1') -DataRoot $validationData -RemoveData -RemoveBackups -RemoveLogs -Confirmation 'REMOVER DADOS CLOUDOS';Record 18 'desinstalar removendo dados temporários' 'PASS'
