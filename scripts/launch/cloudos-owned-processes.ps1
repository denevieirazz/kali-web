...
function Get-CloudOSOwnedSessionManifest {
    ...
    if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
        throw "SESSION_OWNERSHIP_MANIFEST_MISSING:$manifestPath"
    }

    try {
        $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
    } catch {
        throw "SESSION_OWNERSHIP_MANIFEST_INVALID:$manifestPath:$($_.Exception.Message)"
    }

    if ([string]$manifest.id -ne [string]$Session.id) {
        throw "SESSION_OWNERSHIP_MANIFEST_ID_MISMATCH:expected=$($Session.id):actual=$($manifest.id)"
    }
    if ([string]$manifest.logDirectory -ne [string]$Session.logDirectory) {
        throw 'SESSION_OWNERSHIP_MANIFEST_LOG_DIRECTORY_MISMATCH'
    }
...
function Stop-CloudOSRecordedProcesses {
    ...
    foreach ($record in @($manifest.processes) | Sort-Object { [int]$_.pid } -Descending) {
        if (-not $record.pid -or -not $record.startedAt -or -not $record.component) {
            throw "SESSION_OWNERSHIP_RECORD_INCOMPLETE:$($ownership.path)"
        }
        ...
        try {
            ...
            if (-not $process.WaitForExit(5000)) {
                ...
                if (-not $process.WaitForExit(3000)) {
                    throw "OWNED_PROCESS_DID_NOT_EXIT:$($record.component):$($record.pid)"
                }
            }
        } catch {
            throw "SESSION_TEARDOWN_FAILED:$($record.component):$($record.pid):$($_.Exception.Message)"
        }
    }
}
