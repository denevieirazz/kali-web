...
- Quando uma alteração estiver pronta para o mantenedor baixar e testar, não deixe o resultado final apenas em branch ou PR: depois que os gates obrigatórios acessíveis estiverem verdes, faça merge na `main` e informe o SHA resultante. Se ainda faltar validação física que só pode ocorrer na máquina do mantenedor, deixe essa pendência explícita, mas mantenha a `main` como o snapshot integrado que ele deve baixar/testar.
