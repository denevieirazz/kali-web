...
function StartMenu() {
  ...
  const context = (event:React.MouseEvent, app:App) => {
    ...
    openContextMenu(event.clientX, event.clientY, [{ id:'open', label:'Abrir', icon:'âš¡', onClick:() => launch(app) }]);
  };
  ...
  return <div className="start-menu-overlay">
    <div ref={menuRef} className="start-menu acrylic" role="dialog" aria-label="Menu Iniciar">
      <div className="start-search"><span className="start-search-icon">âŒ•</span><input ref={inputRef} className="start-search-input" placeholder="Pesquisar aplicativos..." value={query} onChange={e => setQuery(e.target.value)} /></div>
      {!query && <nav className="start-native-tabs" aria-label="SeÃ§Ãµes"><button className={view==='home'?'active':''} onClick={() => setView('home')}>InÃ­cio</button><button className={view==='all'?'active':''} onClick={() => setView('all')}>Todos</button><button className={view==='running'?'active':''} onClick={() => setView('running')}>Abertos <b>{windows.length}</b></button></nav>}
      <main className="start-native-content">
        {query ? <AppGrid apps={filtered} launch={launch} context={context}/> : view==='home' ? <><div className="start-section-header"><strong>Fixados</strong><button onClick={() => setView('all')}>Todos os apps â†’</button></div><AppGrid apps={appList.slice(0, 12)} launch={launch} context={context}/></> : view==='all' ? <AppGrid apps={appList} launch={launch} context={context}/> : <section className="start-running"><header><div><strong>Aplicativos abertos</strong><small>{windows.length} janela{windows.length===1?'':'s'}</small></div><button className="close-all" disabled={!windows.length} onClick={closeAll}>Fechar todas</button></header>{windows.length ? <div className="running-list">{windows.map(window => <article className="running-item" key={window.id}><button className="running-main" onClick={() => activate(window.id)}><span className="running-icon">{window.icon || 'â–£'}</span><span><strong>{window.title || window.appId}</strong><small>{window.isMinimized?'Minimizada':window.isActive?'Ativa':'Em execuÃ§Ã£o'}</small></span></button><div className="running-actions"><button onClick={() => minimizeWindow(window.id)} title="Minimizar">âˆ’</button><button onClick={() => toggleMaximize(window)} title="Maximizar ou restaurar">â–¡</button><button className="danger" onClick={() => closeWindow(window.id)} title="Fechar">Ã—</button></div></article>)}</div> : <div className="start-empty">Nenhum aplicativo aberto.</div>}</section>}
      </main>
      <footer className="start-bottom"><div className="start-user-btn"><div className="start-user-avatar">{currentUser?.avatar ? <img src={currentUser.avatar} alt=""/> : 'â—'}</div><span className="start-user-name">{currentUser?.displayName || currentUser?.username || 'UsuÃ¡rio'}</span></div><button className="start-power-btn" onClick={() => window.location.reload()} title="Reiniciar interface">â»</button></footer>
    </div>
  </div>;
}
