...
function passwordStrength(password: string) {
  ...
  if (password.length < 4) return { level: 'invalid', label: 'Muito curta', detail: 'O mínimo desta fase é 4 caracteres.' };
  if (password.length < 8) return { level: 'weak', label: 'Fraca, mas permitida', detail: 'Recomendamos uma frase maior, mesmo sem números ou símbolos obrigatórios.' };
...
export default function SetupWizard() {
  ...
  useEffect(() => { if (setupStatus === 'checking') void checkSetupStatus(); }, [checkSetupStatus, setupStatus]);
  useEffect(() => {
    ...
    <motion.div initial={{ scale: .96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="setup-container">
      ...
      <main className="setup-right">
        ...
        <div className="setup-content"><AnimatePresence mode="wait">
          ...
          : step === 'account' ? <motion.form key="account" className="setup-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={event => { event.preventDefault(); goNext(); }}><span className="setup-kicker">CONTA ADMINISTRADORA</span><h1 className="setup-title compact">Identifique-se</h1><label className="setup-field">Nome de exibição<input className="setup-input" value={displayName} onChange={event => setDisplayName(event.target.value)} autoComplete="name" maxLength={80} placeholder="Como você quer ser chamado" /></label><label className="setup-field">Nome de usuário<input className="setup-input" value={username} onChange={event => setUsername(event.target.value)} autoComplete="username" maxLength={64} placeholder="exemplo.usuario" /></label><div className="setup-password-grid"><label className="setup-field">Senha<input type="password" className="setup-input" value={password} onChange={event => setPassword(event.target.value)} autoComplete="new-password" minLength={4} maxLength={128} /></label><label className="setup-field">Confirmar senha<input type="password" className="setup-input" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} autoComplete="new-password" minLength={4} maxLength={128} /></label></div><div className={`setup-password-strength setup-password-strength--${strength.level}`} data-password-strength={strength.level}><strong>{strength.label}</strong><span>{strength.detail}</span></div><small className="setup-field-help">Mínimo de 4 caracteres. Espaços e frases-senha são aceitos. Não exigimos maiúsculas, números ou símbolos.</small><button type="submit" hidden aria-hidden="true" /></motion.form>
        ...
        {!unavailable && <footer className="setup-footer">{currentIndex > 0 && step !== 'recovery' && <button className="setup-btn setup-btn-secondary" onClick={() => { setError(null); setStep(STEPS[currentIndex - 1]); }}>Voltar</button>}{step !== 'recovery' ? <button className="setup-btn setup-btn-primary" onClick={goNext} disabled={loading}>{loading ? 'Criando conta…' : step === 'theme' ? 'Criar conta' : 'Continuar'}</button> : <button className="setup-btn setup-btn-primary" onClick={() => finishSetup(false)} disabled={!recoverySaved}>Entrar no CloudOS</button>}</footer>}
      </main>
    </motion.div>
  </div>;
}
