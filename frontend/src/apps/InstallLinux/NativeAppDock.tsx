...
export default function NativeAppDock({
  ...
  useEffect(() => {
    ...
    const synchronize = async () => {
      ...
      if (useNativeWindowBindings.getState().sessionToWindow[session.sessionId]) {
        controlledSessionsRef.current.delete(session.sessionId);
        lastLayoutRef.current.delete(session.sessionId);
        return;
      }
