...
export default function InstallLinux() {
  ...
  const hubNativeSessions = useMemo(
    () => nativeSessions.filter((session) => !nativeWindowBindings[session.sessionId]),
    [nativeSessions, nativeWindowBindings],
  );
  ...
  useEffect(() => {
    setSelectedNativeSessionId((current) => {
      if (current && hubNativeSessions.some((session) => session.sessionId === current)) return current;
      return hubNativeSessions[0]?.sessionId || null;
    });
  }, [hubNativeSessions]);

  useEffect(() => {
    if (!pendingNativeLaunch) return;
    const session = nativeSessionForLaunch(hubNativeSessions, pendingNativeLaunch.launch);
    if (session) {
  ...
  }, [hubNativeSessions, pendingNativeLaunch]);
