...
export default function SettingsApp({}: { windowId: string }) {
  ...
  const [secondaryCreating,setSecondaryCreating]=useState(false);
  const [secondaryRecoveryCode,setSecondaryRecoveryCode]=useState<string|null>(null);
  const [secondaryRecoverySaved,setSecondaryRecoverySaved]=useState(false);
  ...
  async function createSecondaryAccount(event:React.FormEvent){
    event.preventDefault();
    setSecondaryNotice(null);
    const validation=validateUsername(secondaryUsername)||validateDisplayName(secondaryDisplayName,{required:false})||validateNewPassword(secondaryPassword,secondaryConfirmPassword);
    if(validation){setSecondaryNotice(validation);return}
    setSecondaryCreating(true);
    const result=await createAccount(secondaryUsername.trim(),secondaryDisplayName.trim()||secondaryUsername.trim(),secondaryPassword,secondaryConfirmPassword);
    setSecondaryCreating(false);
    setSecondaryPassword('');setSecondaryConfirmPassword('');
    if(!result.success||!result.recoveryCode){setSecondaryNotice(result.message||'Não foi possível criar a conta.');return}
    setSecondaryUsername('');setSecondaryDisplayName('');setSecondaryRecoverySaved(false);setSecondaryRecoveryCode(result.recoveryCode);
  }
  async function copySecondaryRecoveryCode(){if(!secondaryRecoveryCode)return;try{await navigator.clipboard.writeText(secondaryRecoveryCode)}catch{setSecondaryNotice('Selecione e copie o código manualmente.')}}
  ...
  function renderAccounts(){return <><h2>Contas</h2><div className="settings-hero"><div className="settings-avatar">{String(currentUser?.displayName||currentUser?.username||'U').slice(0,1).toUpperCase()}</div><div><h3>{currentUser?.displayName||currentUser?.username||'Usuário local'}</h3><p>{currentUser?.role==='admin'?'Conta administrativa local do CloudOS':'Conta de usuário padrão do CloudOS'}</p></div></div><div className="settings-card"><Row label="Nome de usuário" value={currentUser?.username||'-'}/><Row label="Tipo" value={currentUser?.role==='admin'?'Administrador local':'Usuário padrão'}/><button className="settings-secondary" onClick={()=>kernel.sysLock()}>Bloquear sessão</button></div>{currentUser?.role==='admin'&&<div className="settings-card"><h3>Criar conta local</h3><p className="settings-recovery-description">A criação exige esta sessão administrativa. A nova conta receberá seu próprio armazenamento e código de recuperação.</p>{secondaryNotice&&<div className="settings-recovery-notice">{secondaryNotice}</div>}{secondaryRecoveryCode?<div className="settings-recovery-result"><strong>Código da nova conta — mostrado uma única vez</strong><div><code>{secondaryRecoveryCode}</code><button className="settings-secondary" onClick={copySecondaryRecoveryCode}>Copiar</button></div><label><input type="checkbox" checked={secondaryRecoverySaved} onChange={e=>setSecondaryRecoverySaved(e.target.checked)}/><span>Confirmei que entreguei ou salvei o código</span></label><button className="settings-action" disabled={!secondaryRecoverySaved} onClick={()=>{setSecondaryRecoveryCode(null);setSecondaryRecoverySaved(false);setSecondaryNotice('Conta criada. Ela já pode entrar pela tela de bloqueio.')}}>Concluir</button></div>:<form className="settings-account-form" onSubmit={createSecondaryAccount}><label>Nome de usuário<input value={secondaryUsername} onChange={e=>setSecondaryUsername(e.target.value)} autoComplete="off"/></label><label>Nome de exibição (opcional)<input value={secondaryDisplayName} onChange={e=>setSecondaryDisplayName(e.target.value)} autoComplete="off"/></label><label>Senha<input type="password" value={secondaryPassword} onChange={e=>setSecondaryPassword(e.target.value)} autoComplete="new-password"/></label><label>Confirmar senha<input type="password" value={secondaryConfirmPassword} onChange={e=>setSecondaryConfirmPassword(e.target.value)} autoComplete="new-password"/></label><button className="settings-action" type="submit" disabled={secondaryCreating}>{secondaryCreating?'Criando…':'Criar conta'}</button></form>}</div>}<div className="settings-card"><h3>Código de recuperação</h3><p className="settings-recovery-description">Gere um código novo se a conta foi criada antes deste recurso ou se o código anterior não está mais seguro. O código antigo será invalidado.</p>{accountNotice&&<div className="settings-recovery-notice">{accountNotice}</div>}{rotatedRecoveryCode?<div className="settings-recovery-result"><strong>Mostrado uma única vez</strong><div><code>{rotatedRecoveryCode}</code><button className="settings-secondary" onClick={copyAccountRecoveryCode}>Copiar</button></div><label><input type="checkbox" checked={recoveryCodeSaved} onChange={e=>setRecoveryCodeSaved(e.target.checked)}/><span>Confirmei que salvei o novo código</span></label><button className="settings-action" disabled={!recoveryCodeSaved} onClick={()=>{confirmRecoveryCodeSaved();rotatedRecoveryCodeRef.current=null;setRotatedRecoveryCode(null);setRecoveryCodeSaved(false);setAccountNotice('Código salvo. Ele não ficará armazenado nesta interface.')}}>Fechar código</button></div>:<button className="settings-action" disabled={rotatingRecoveryCode} onClick={rotateAccountRecoveryCode}>{rotatingRecoveryCode?'Gerando…':'Gerar novo código'}</button>}</div></>}
