...
import { buildQuickWebEvidence, buildQuickWebMarkdown } from './quickWebEvidence';
...
type WebInspection = {
  ...
  headers: Record<string, string>; cookies: WebCookie[];
  ...
  nextSteps: string[]; completedAt: string; durationMs: number;
};
...
function normalizeInput(value: string) { const trimmed = value.trim(); return /^https?:\/\//i.test(trimmed) ? trimmed : ''; }
function safeHost(value: string) { try { return new URL(value).hostname.replace(/[^a-z0-9.-]+/gi, '_').slice(0, 100) || 'web'; } catch { return 'web'; } }
function downloadText(filename: string, body: string, type: string) {
  const href = URL.createObjectURL(new Blob([body], { type })); const anchor = document.createElement('a'); anchor.href = href; anchor.download = filename; anchor.click();
...
export default function QuickWebChecks() {
  ...
  const run = async (selectedMode: Mode) => {
    ...
    try {
      const data = await apiClient<WebInspection>('/api/security/tools/web/inspect', { method: 'POST', timeoutMs: 55_000, body: JSON.stringify({ url: target }) });
    ...
    } catch (cause) { setResult(null); setError(cause instanceof Error ? cause.message : 'A análise web não foi concluída.'); }
    finally { setLoading(''); }
  };
  ...
  const evidence = useMemo(() => result ? buildQuickWebEvidence(result) : null, [result]);

  const copyForAi = async () => {
    if (!evidence) return;
    try { await navigator.clipboard.writeText(JSON.stringify(evidence, null, 2)); setNotice('Contexto web copiado para a IA.'); }
    catch { setError('Não foi possível copiar para a área de transferência.'); }
  };
  const exportJson = () => {
    if (!result || !evidence) return;
    downloadText(`cloudos-web-${safeHost(result.finalUrl)}-${Date.now()}.json`, JSON.stringify(evidence, null, 2), 'application/json');
    setNotice('Evidência web JSON salva.');
  };
  const exportMarkdown = () => {
    if (!result) return;
    downloadText(`cloudos-web-report-${safeHost(result.finalUrl)}-${Date.now()}.md`, buildQuickWebMarkdown(result), 'text/markdown;charset=utf-8');
    setNotice('Relatório web Markdown salvo.');
  };
  ...
  return <section className="qwc-root" aria-label="Checks web de um clique">
    ...
    {result && <section className="qwc-result">
      <header><div><small>{new URL(result.finalUrl).hostname}</small><strong>HTTP {result.status} · {result.title || 'sem título'}</strong><span>{result.finalUrl}</span></div><div className="qwc-result-actions"><button type="button" onClick={() => void copyForAi()}>Copiar para IA</button><button type="button" onClick={exportJson}>Exportar JSON</button><button type="button" onClick={exportMarkdown}>Salvar relatório</button></div></header>
