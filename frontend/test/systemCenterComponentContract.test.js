...
test('System Center boots on Linux real and binds the selected source to the DOM attribute',()=>{assert.match(source,/useState<SystemCenterSource>\('linux-real'\)/);assert.match(source,/data-system-center-source=\{source\}/);assert.match(source,/select value=\{source\}/);});
test('Linux readiness text is conditional and unavailable state remains visible instead of blocking render',()=>{assert.match(source,/linuxStatus\?\.available\?'WSL Core v2':'Linux indisponível'/);assert.match(source,/linuxError&&<div className="sc-state sc-state--error" role="alert">/);});
