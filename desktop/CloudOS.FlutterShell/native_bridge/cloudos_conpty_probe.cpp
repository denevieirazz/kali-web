...
public:
        ...
        std::string& error)
    {
        ...
        if (shell == "powershell")
        {
            // Match xterm's interactive input stream instead of presenting the
            // entire command as one synthetic paste to PSReadLine.
            for (const char ch : input)
            {
                if (!manager.WriteSession(session_id, std::string(1, ch)))
                {
        ...
        else if (!manager.WriteSession(session_id, input))
        {
            error = "WriteSession failed";
            manager.CloseSession(session_id);
            return false;
...
int main()
{
    ...
    const bool powershell_ok = probe.RunCommand(
        "powershell",
        "",
        "Get-Location; Write-Output __CLOUDOS_PS_DONE__; exit\r",
        "__CLOUDOS_PS_DONE__",
        powershell_output,
        powershell_error);
