    ...
    public sealed class NativeWindowManager : IDisposable
    {
            ...
            out string error)
        {
            ...
            try
            {
                ...
                // Windows normalizes dependent non-client bits when a top-level frame is
                // removed. Chromium windows, for example, automatically clear
                // WS_EX_WINDOWEDGE. Exact equality therefore rejects a successfully docked
                // window. Validate only the invariants required by CloudOS containment.
                if (actualOwner != owner
                    || !HasRequiredContainmentStyles(actualStyle, actualExtendedStyle))
                {
                    throw new Win32Exception(
                        Marshal.GetLastWin32Error(),
                        "Windows refused the CloudOS containment owner or required frame styles.");
                }
        ...
        internal static bool HasRequiredContainmentStyles(long style, long extendedStyle)
        {
            long forbiddenFrameStyles = NativeMethods.WS_CAPTION
                ...
                | NativeMethods.WS_MAXIMIZEBOX
                | NativeMethods.WS_SYSMENU;
            return (style & forbiddenFrameStyles) == 0
                && (extendedStyle & NativeMethods.WS_EX_APPWINDOW) == 0
        ...
        private bool TryCreateSnapshot(IntPtr hwnd, out NativeWindowSnapshot snapshot)
        {
            ...
            // Muitos programas criam primeiro uma HWND provisória, vazia e sem
            // título, destruída assim que a janela principal fica pronta. Anexar
            // essa superfície produz exatamente o painel preto "Aplicativo PID"
            // e perde o vínculo alguns segundos depois. O evento NAMECHANGE ou a
            // varredura periódica adicionará a janela quando ela estiver estável.
            string title = NativeMethods.GetWindowTitle(hwnd, _options.MaxTitleLength);
            if (!isAttached && string.IsNullOrWhiteSpace(title)) return false;
