...
var tests = new (string Name, Action Run)[]
{
    ...
    ("Chromium normalized frame remains containable", ChromiumNormalizedFrameRemainsContainable)
};
...
static void ChromiumNormalizedFrameRemainsContainable()
{
    // Brave removes WS_EX_WINDOWEDGE (0x100) after CloudOS removes its caption.
    // The remaining style still has every invariant needed for containment.
    Assert(NativeWindowManager.HasRequiredContainmentStyles(0x16000000, 0x200080),
        "A normalized Chromium frame must remain accepted.");
    Assert(!NativeWindowManager.HasRequiredContainmentStyles(0x16CF0000, 0x200080),
        "A window that retained its external frame must be rejected.");
    Assert(!NativeWindowManager.HasRequiredContainmentStyles(0x16000000, 0x240080),
        "A window that remains an app-window must be rejected.");
}
