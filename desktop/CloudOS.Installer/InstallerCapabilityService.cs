...
public sealed class InstallerCapabilityService : IDisposable
{
    ...
    private static void TryDeleteDirectory(string directory)
    {
        ...
        for (var attempt = 0; attempt < maxAttempts; attempt++)
        {
            ...
            {
                // Windows can keep the executable image section alive for a short time
                // after the containment Job reports zero members. Retry within a small,
                // bounded window so normal completion actually removes staging while the
                // startup stale-directory purge remains the final recovery path.
                if (attempt + 1 < maxAttempts) Thread.Sleep(25 * (attempt + 1));
            }
        }
    }
