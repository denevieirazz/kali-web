...
std::vector<std::wstring> DesktopFiles()
{
    ...
    for (std::filesystem::directory_iterator iterator(desktop, error), end;
         !error && iterator != end && result.size() < 24;
    ...
    CloudOSNativeWindowManager* window_manager)
{
    ...
    if (!NativeDesktopDropTarget::Register(hwnd_))
    {
        // Drag/drop is additive; the shell can still run if OLE registration is unavailable.
    }
    return true;
}
...
void CloudOSNativeDesktopWindow::Destroy()
{
    ...
    quick_launch_app_indices_.clear();
    g_file_rects.clear();
    g_file_paths.clear();
}
...
void CloudOSNativeDesktopWindow::UpdateLayout(const RECT& work_area)
{
    ...
    SetWindowPos(
        ...
        work_area.left,
        work_area.top,
        std::max(1, Width(work_area)),
        std::max(1, Height(work_area)),
...
void CloudOSNativeDesktopWindow::ActivateAppIndex(int app_index)
{
    ...
    {
        NativeAppLauncher::Launch(instance_, hwnd_, kAllApps[static_cast<std::size_t>(app_index)]);
    }
}
...
void CloudOSNativeDesktopWindow::Paint()
{
    ...
    if (!NativeWallpaperManager::Draw(graphics, width, height))
    {
        ...
        SolidBrush glow(Color(35, 74, 132, 210));
        graphics.FillEllipse(
            &glow,
            static_cast<REAL>(width - Scale(820, dpi)),
            static_cast<REAL>(-Scale(320, dpi)),
            static_cast<REAL>(Scale(920, dpi)),
            static_cast<REAL>(Scale(720, dpi)));
    }

    SolidBrush overlay(Color(40, 0, 0, 0));
    graphics.FillRectangle(&overlay, RectF(0.0f, 0.0f, static_cast<REAL>(width), static_cast<REAL>(height)));

    Font brand_font(L"Segoe UI Variable Display", static_cast<REAL>(Scale(23, dpi)), FontStyleBold, UnitPixel);
    Font small_font(L"Segoe UI", static_cast<REAL>(Scale(9, dpi)), FontStyleRegular, UnitPixel);
    Font small_bold(L"Segoe UI", static_cast<REAL>(Scale(9, dpi)), FontStyleBold, UnitPixel);
    SolidBrush white(Color(255, 245, 247, 250));
    SolidBrush secondary(Color(240, 208, 214, 224));
    ...
    graphics.DrawString(
        ...
        -1,
        &brand_font,
        PointF(static_cast<REAL>(primary.x + Scale(24, dpi)), static_cast<REAL>(primary.y + Scale(20, dpi))),
        &white);
    ...
    graphics.DrawString(
        ...
        -1,
        &small_font,
        PointF(static_cast<REAL>(primary.x + Scale(26, dpi)), static_cast<REAL>(primary.y + Scale(54, dpi))),
        &secondary);

    quick_launch_rects_.clear();
    const int shortcut_width = Scale(92, dpi);
    const int shortcut_height = Scale(88, dpi);
    const int icon_size = Scale(44, dpi);
    const int left = primary.x + Scale(20, dpi);
    int y = primary.y + Scale(92, dpi);
    ...
    for (std::size_t index = 0; index < quick_launch_app_indices_.size(); ++index)
    {
        ...
        NativeIconRenderer::DrawAetherSquircle(
            ...
            left + (shortcut_width - icon_size) / 2,
            y + Scale(3, dpi),
            icon_size);
        DrawCenteredText(
            graphics,
            ...
            RectF(
                ...
                static_cast<REAL>(y + Scale(53, dpi)),
                static_cast<REAL>(shortcut_width),
                static_cast<REAL>(Scale(30, dpi))),
            white);
    ...
    g_file_rects.clear();
    const int file_start_x = left + shortcut_width + Scale(24, dpi);
    const int file_start_y = primary.y + Scale(92, dpi);
    const int file_columns = 5;
    const int file_cell_width = Scale(110, dpi);
    const int file_cell_height = Scale(96, dpi);
    ...
    for (std::size_t index = 0; index < g_file_paths.size(); ++index)
    {
        ...
        if (SHGetFileInfoW(
                g_file_paths[index].c_str(),
                0,
                &file_info,
                sizeof(file_info),
                SHGFI_ICON | SHGFI_LARGEICON) != 0 && file_info.hIcon != nullptr)
        {
            DrawIconEx(
                memory_dc,
                x + (file_cell_width - Scale(40, dpi)) / 2,
                file_y + Scale(4, dpi),
                file_info.hIcon,
                Scale(40, dpi),
                Scale(40, dpi),
        ...
        DrawCenteredText(
            graphics,
            FileName(g_file_paths[index]),
            small_font,
            RectF(
                static_cast<REAL>(x + Scale(3, dpi)),
                static_cast<REAL>(file_y + Scale(50, dpi)),
                static_cast<REAL>(file_cell_width - Scale(6, dpi)),
                static_cast<REAL>(Scale(40, dpi))),
            white);
    }
    ...
    graphics.DrawString(
        ...
        -1,
        &small_bold,
        PointF(static_cast<REAL>(primary.x + Scale(24, dpi)), static_cast<REAL>(primary.y + Scale(72, dpi))),
        &secondary);
    ...
    LPARAM l_param)
{
    ...
    case WM_LBUTTONDBLCLK:
    {
        ...
        for (std::size_t index = 0; index < quick_launch_rects_.size() && index < quick_launch_app_indices_.size(); ++index)
        {
            ...
            {
                ActivateAppIndex(quick_launch_app_indices_[index]);
                return 0;
            }
        }
        for (std::size_t index = 0; index < g_file_rects.size() && index < g_file_paths.size(); ++index)
        {
            ...
            {
                (void)ShellExecuteW(hwnd_, L"open", g_file_paths[index].c_str(), nullptr, nullptr, SW_SHOWNORMAL);
                return 0;
            }
        }
        break;
    ...
    case WM_RBUTTONUP:
    {
        ...
        if (NativeDesktopContextMenu::Show(instance_, hwnd_, point))
        {
            Redraw();
        }
        return 0;
