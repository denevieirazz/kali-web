...
// Research/reuse note:
// - Hosting pattern follows Microsoft's MIT-licensed Windows classic sample
//   ExplorerBrowserCustomContents.
// - CLSID_ExplorerBrowser is a native Windows Shell COM view. It is NOT
//   WebView/WebView2/HTML and does not reparent external application HWNDs.
// Reference:
...
NativeShellViewHost::EventSink::EventSink(
    NativeShellViewHost* owner) noexcept
    : owner_(owner)
{
}

HRESULT STDMETHODCALLTYPE NativeShellViewHost::EventSink::QueryInterface(
    REFIID iid,
    void** object)
{
    if (object == nullptr)
...
HRESULT STDMETHODCALLTYPE NativeShellViewHost::EventSink::OnNavigationPending(
    PCIDLIST_ABSOLUTE)
{
    return S_OK;
}

HRESULT STDMETHODCALLTYPE NativeShellViewHost::EventSink::OnViewCreated(
...
HRESULT STDMETHODCALLTYPE NativeShellViewHost::EventSink::OnNavigationComplete(
    PCIDLIST_ABSOLUTE folder)
{
    if (owner_ != nullptr)
    {
        owner_->OnNavigationComplete(folder);
    }
    return S_OK;
}

HRESULT STDMETHODCALLTYPE NativeShellViewHost::EventSink::OnNavigationFailed(
    PCIDLIST_ABSOLUTE)
{
    return S_OK;
}

NativeShellViewHost::~NativeShellViewHost()
{
    Destroy();
}

bool NativeShellViewHost::Create(
    HWND parent,
    const RECT& bounds,
    NavigationCallback navigation_callback)
{
    ...
    const EXPLORER_BROWSER_OPTIONS options =
        static_cast<EXPLORER_BROWSER_OPTIONS>(
            EBO_ALWAYSNAVIGATE |
            EBO_NOBORDER);
    ...
    if (sink != nullptr)
    {
        if (FAILED(browser->Advise(sink, &cookie)))
        {
            cookie = 0;
        }
        sink->Release();
...
void NativeShellViewHost::Resize(const RECT& bounds) noexcept
{
    if (browser_ != nullptr)
    {
        (void)browser_->SetRect(nullptr, bounds);
    }
}

bool NativeShellViewHost::IsReady() const noexcept
{
    return browser_ != nullptr;
}

const std::wstring& NativeShellViewHost::CurrentPath() const noexcept
{
...
{
    if (browser_ == nullptr || path.empty())
    {
        return false;
    }

    PIDLIST_ABSOLUTE item_id_list = nullptr;
    const HRESULT parse_result = SHParseDisplayName(
        path.c_str(),
        nullptr,
        &item_id_list,
        0,
        nullptr);
    if (FAILED(parse_result) || item_id_list == nullptr)
    {
        return false;
    }

    const HRESULT browse_result = browser_->BrowseToIDList(
        item_id_list,
        SBSP_ABSOLUTE);
...
{
    return browser_ != nullptr &&
        SUCCEEDED(browser_->BrowseToIDList(nullptr, SBSP_NAVIGATEBACK));
}

bool NativeShellViewHost::NavigateForward()
{
    return browser_ != nullptr &&
        SUCCEEDED(browser_->BrowseToIDList(nullptr, SBSP_NAVIGATEFORWARD));
}

bool NativeShellViewHost::NavigateParent()
{
    return browser_ != nullptr &&
        SUCCEEDED(browser_->BrowseToIDList(nullptr, SBSP_PARENT));
}

bool NativeShellViewHost::Refresh()
{
    IShellView* view = nullptr;
    if (!GetCurrentView(&view) || view == nullptr)
    {
        return false;
    }
...
std::wstring NativeShellViewHost::DisplayPathFromPidl(
    PCIDLIST_ABSOLUTE folder)
{
    if (folder == nullptr)
    {
...
void NativeShellViewHost::OnNavigationComplete(
    PCIDLIST_ABSOLUTE folder)
{
    current_path_ = DisplayPathFromPidl(folder);
    if (navigation_callback_)
    {
        navigation_callback_(current_path_);
    }
}
