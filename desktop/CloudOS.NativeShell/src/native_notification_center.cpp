...
{
    if (window_ != nullptr) RebuildList();
}
...
void CloudOSNativeNotificationCenter::RebuildList()
{
    ...
    for (std::size_t index = 0; index < snapshot.size(); ++index)
    {
        ...
        ListView_SetItemText(list_, static_cast<int>(index), 1, const_cast<LPWSTR>(text.c_str()));
    }
...
void CloudOSNativeNotificationCenter::Layout()
{
    ...
    MoveWindow(list_, margin, margin, std::max(1, width - margin * 2),
        std::max(1, height - margin * 3 - footer), TRUE);
    MoveWindow(status_label_, margin, height - margin - Scale(30, dpi), Scale(220, dpi), Scale(30, dpi), TRUE);
...
void CloudOSNativeNotificationCenter::ShowNear(const RECT& anchor)
{
    ...
    const UINT dpi = GetDpiForWindow(window_);
    const int width = Scale(480, dpi);
    const int height = Scale(580, dpi);
...
LRESULT CloudOSNativeNotificationCenter::HandleMessage(HWND window, UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    case WM_COMMAND:
        ...
        {
            { std::scoped_lock lock(g_notification_mutex); g_notifications.clear(); }
            RebuildList();
            return 0;
        }
        break;
