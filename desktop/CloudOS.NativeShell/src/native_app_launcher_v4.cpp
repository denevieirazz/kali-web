...
std::wstring CanonicalAppId(std::wstring_view id)
{
    ...
    if (id == L"workspace" || id == L"overview" || id == L"task-view" || id == L"mission-control" || id == L"areas") return L"workspaces";
    if (id == L"system-center" || id == L"hardware" || id == L"hardware-center" || id == L"systemcontrol" || id == L"controle-sistema") return L"systemcenter";
    ...
    const std::wstring& requested_id)
{
    ...
    else if (id == L"systemcenter")
    {
        launched = CloudOSNativeSystemControlWindow::Open(instance, parent_hwnd) != nullptr;
        if (!launched) ShowLaunchError(parent_hwnd, L"a Central do Sistema");
    }
    ...
    {
        launched = LaunchWindowsTarget(parent_hwnd, L"explorer.exe");
    }
    else if (id == L"drive")
    {
        std::wstring error;
...
void NativeAppLauncher::ShowQuickPowerMenu(HWND parent_hwnd, POINT screen_pt)
{
    ...
    constexpr UINT kSystemCenter = 1143;
    ...
    if (menu == nullptr || cloud == nullptr || terminals == nullptr || productivity == nullptr ||
        tools == nullptr || settings == nullptr || power == nullptr)
    {
    ...
    AppendMenuW(menu, MF_STRING, kSystemCenter, L"Central do Sistema  ·  hardware e rede");
    ...
    AppendMenuW(tools, MF_STRING, kSystemCenter, L"Central do Sistema");
    ...
    case kSystemCenter: LaunchById(instance, parent_hwnd, L"systemcenter"); break;
