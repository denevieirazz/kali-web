...
25. **Interface Burp Suite & VS Code para Tool Runner (V2.1)**: Atualização do `ToolRunnerApp.jsx` com suporte a layout flexbox puro (sem `className` dentro dos objetos de estilo), renderização correta dos painéis de opções e terminal de saída virtual, além de compatibilidade estrita com `payload.toolId` e exports duplos.
