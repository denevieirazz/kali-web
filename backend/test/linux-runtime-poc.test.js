...
test('POC1 allowlist remains intentionally small and blocks arbitrary commands', () => { assert.deepEqual(getAllowedLinuxPocApps().map(item => item.id), ['xclock', 'xeyes', 'xterm', 'gedit']); assert.equal(normalizePocApp('xclock'), 'xclock'); assert.equal(normalizePocApp('Firefox'), null); assert.equal(normalizePocApp('sh -c calc.exe'), null); });
test('B-01: Xpra loopback socket requires a per-session capability password', () => { const command = buildXpraStartCommand({ appCommand: 'xclock', port: 14500, sessionId: 'test-session', password: '0123456789abcdef0123456789abcdef' }); assert.match(command, /export XPRA_PASSWORD=/); assert.match(command, /--bind-tcp=127\.0\.0\.1:14500,auth=env/); assert.doesNotMatch(command, /auth=allow/); assert.match(command, /--start-new-commands=no/); assert.doesNotMatch(command, /0\.0\.0\.0/); assert.equal(displayForPort(14549), 149); });
