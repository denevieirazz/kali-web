...
function brokerPolicy(name, target = '') {
  const identity = `${name || ''} ${target || ''}`.toLocaleLowerCase('pt-BR');
  if (/calculadora|calculator|calc\.exe|microsoft\.windowscalculator/.test(identity)) {
    return { fallbackAppId: 'calculator' };
  }
  if (/\bpaint\b|mspaint\.exe|microsoft\.paint/.test(identity)) {
    return {
      availability: 'blocked',
      blockedReason: 'O Paint moderno usa um broker do Windows e não pode ser contido com segurança nesta versão do CloudOS.'
    };
  }
  return {};
}
...
function publicApp(app) {
  ...
  const brokeredInManagedHost = mode === 'native-managed' && app.kind === 'windows-start-app';
  const availability = app.fallbackAppId
    ? 'fallback'
    : app.availability === 'blocked' || brokeredInManagedHost
      ? 'blocked'
      : 'contained';
  return {
    id: app.id,
    ...
    windowMode: mode,
    availability,
    fallbackAppId: app.fallbackAppId || null,
    blockedReason: availability === 'blocked'
      ? app.blockedReason || 'Este aplicativo usa um broker do Windows e não pode ser aberto sem sair da contenção do CloudOS.'
      : null
  };
}
...
function normalizedShortcutArguments(value) {
  return typeof value === 'string' ? value.trim() : '';
}

function shortcutIdentity(targetPath, argumentsValue = '') {
  return `${String(targetPath || '').trim().toLowerCase()}\0${normalizedShortcutArguments(argumentsValue)}`;
}

function curatedShortcutArguments(app) {
  if (typeof app?.arguments === 'string') return app.arguments;
  return Array.isArray(app?.args) ? app.args.map((argument) => String(argument)).join(' ') : '';
}

export function parseWindowsAppDiscovery(payload, curatedApps = []) {
  const directIdentities = new Set(curatedApps
    .filter((app) => app?.executable)
    .map((app) => shortcutIdentity(app.executable, curatedShortcutArguments(app))));
  ...
  for (const row of asArray(payload?.Shortcuts).slice(0, 1200)) {
    ...
    const argumentsValue = normalizedShortcutArguments(row.Arguments);
    if (!name || name.includes('\0') || shortcutPath.includes('\0') || targetPath.includes('\0') || argumentsValue.includes('\0')) continue;
    ...
    const identityKey = shortcutIdentity(targetPath, argumentsValue);
    const nameKey = normalizedDisplayName(name);
    if (directIdentities.has(identityKey)) continue;
    directIdentities.add(identityKey);
    directNames.add(nameKey);
    shortcuts.push({
      id: opaqueId('windows-shortcut', shortcutPath, targetPath, argumentsValue),
      name,
      source: 'windows',
      distribution: null,
      icon: '\u25a6',
  ...
  const startApps = asArray(payload?.StartApps)
    ...
    .map((row) => ({
      ...
      ...brokerPolicy(row.Name, row.AppID)
    }));
...
async function getWindowsStartApps() {
  ...
  const curatedApps = [
    ...fallbackApps(),
    {
      id: opaqueId('cloudos-fallback', 'calculator'),
      name: 'Calculadora do Windows',
      source: 'windows',
      distribution: null,
      icon: '＋',
      kind: 'cloudos-fallback',
      fallbackAppId: 'calculator'
    }
  ];
  const command = [
...
async function launchWindowsShortcut(app) {
  const command = [
    ...
    'if (-not [StringComparer]::Ordinal.Equals($actualArguments.Trim(), $expectedArguments.Trim())) { throw "Shortcut arguments changed after catalog discovery." }',
  ...
  try {
    const { stdout } = await execFileAsync(POWERSHELL_EXE, ['-NoLogo', '-NoProfile', '-EncodedCommand', encoded], {
      ...
      env: safeChildEnvironment({
        ...
        CLOUDOS_APP_ARGUMENTS: app.arguments || ''
      }),
      timeout: 15_000,
      windowsHide: true,
      maxBuffer: 128 * 1024
...
export async function launchCatalogApp(id) {
  ...
  if (app.fallbackAppId) {
    throw Object.assign(new Error('Use a versão integrada deste aplicativo dentro do CloudOS.'), {
      code: 'APP_CLOUDOS_FALLBACK_REQUIRED',
      fallbackAppId: app.fallbackAppId
    });
  }
  if (app.availability === 'blocked' || (app.kind === 'windows-start-app' && currentWindowMode() === 'native-managed')) {
    throw Object.assign(new Error(app.blockedReason || 'O broker deste aplicativo não permite contenção segura no CloudOS.'), {
      code: 'FAIL_CLOSED_BROKER_DETECTED'
    });
  }
