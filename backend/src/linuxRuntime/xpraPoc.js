...
function publicSession(session) {
  ...
  return { id: session.id, ownerId: session.ownerId, app: session.app, title: session.title, distribution: session.distribution, port: session.port, display: session.display, state: session.state, startedAt: session.startedAt, clientUrl: ['ready', 'degraded'].includes(session.state) ? `${proxyPath(session)}?clipboard=yes&keyboard=yes&printing=no&file_transfer=no&floating_menu=no&reconnect=yes` : null, xpraVersion: session.xpraVersion, error: session.error || null, errorCode: session.errorCode || null, health: session.health || null, metrics: publicMetrics(session) };
}
