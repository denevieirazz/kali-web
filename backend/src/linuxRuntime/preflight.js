  ...
  for (const layer of BOUNDARY_ORDER) {
    ...
    if (!checks.length) {
      result[layer] = upstreamFailed
        ? {
          status: 'FAIL',
          code: `${layer}_NOT_REACHED`,
          component: layer,
          cause: `Camada não alcançada porque ${upstreamFailed} falhou antes.`,
          evidence: `blockedBy=${upstreamFailed}`,
        }
        : {
          status: 'WARN',
          code: `${layer}_PENDING`,
          component: layer,
          cause: 'Camada ainda não executada nesta fase do preflight.',
...
function publicRun(run) {
  ...
  return {
    ...
    ownerId: run.ownerId,
    phase: run.phase,
    decision: run.phase === 'awaiting_iframe' ? 'NO_GO' : decisionFor(run),
...
function prepareEvidenceDirectories(run) {
  ...
  try {
    ...
    addCheck(run, {
      id: 'forensics-directories',
      layer: 'FORENSICS',
      status: 'PASS',
      code: 'EVIDENCE_DIRECTORIES_WRITABLE',
      component: 'screenshots/logs/telemetry',
      cause: 'Diretórios de evidência existem e aceitaram criação/remoção de arquivo de prova.',
      evidence: directories.join(' | '),
    ...
    addCheck(run, {
      ...
      layer: 'FORENSICS',
      status: 'FAIL',
      code: 'EVIDENCE_DIRECTORY_NOT_WRITABLE',
      component: 'screenshots/logs/telemetry',
      cause: 'Não foi possível preparar diretório gravável para a evidência física.',
...
async function captureWindowBaseline(run) {
  ...
  if (process.platform !== 'win32' || !fs.existsSync(POWERSHELL_EXE)) {
    addCheck(run, {
      ...
      layer: 'FORENSICS',
      status: 'FAIL',
      code: 'WINDOW_BASELINE_UNAVAILABLE',
      component: 'Windows HWND baseline',
      cause: 'PowerShell clássico do Windows não está disponível neste host.',
      evidence: `platform=${process.platform}; powershell=${POWERSHELL_EXE}`,
      durationMs: elapsedMs(started),
    });
    return false;
  }
  ...
  try {
    ...
    fs.writeFileSync(WINDOW_BASELINE_FILE, JSON.stringify(payload, null, 2), 'utf8');
    addCheck(run, {
      id: 'window-baseline',
      layer: 'FORENSICS',
      status: 'PASS',
      code: 'WINDOW_BASELINE_CAPTURED',
      component: 'Windows HWND baseline',
      cause: 'Processos com MainWindowHandle foram registrados antes do dry run.',
      evidence: `count=${windows.length}; file=${WINDOW_BASELINE_FILE}`,
      durationMs: elapsedMs(started),
    });
    return true;
  } catch (cause) {
    addCheck(run, {
      ...
      layer: 'FORENSICS',
      status: 'FAIL',
      code: 'WINDOW_BASELINE_CAPTURE_FAILED',
      component: 'Windows HWND baseline',
      cause: 'A captura de processos/janelas/handles falhou antes da prova.',
      evidence: cause.message,
      durationMs: elapsedMs(started),
    });
    return false;
...
async function inspectOrphans(distribution) {
  const ledger = readRuntimeLedger();
...
async function startDryRun(run, pair) {
  ...
  if (!server.ok) {
    addCheck(run, {
      ...
      status: 'FAIL',
      code: server.code,
      component: `Xpra seamless :${session.display}`,
      cause: 'O servidor Xpra efêmero não ficou saudável sem executar aplicativo.',
  ...
  addCheck(run, {
    ...
    layer: 'XPRA',
    status: 'PASS',
    code: 'XPRA_DRY_RUN_SERVER_READY',
    component: `Xpra seamless :${session.display}`,
    cause: 'Servidor Xpra efêmero respondeu a xpra info sem iniciar xclock.',
    evidence: `display=:${session.display}; port=${session.port}; no-start-child=true`,
    durationMs: server.durationMs,
  });
  return true;
}
...
async function validatePostConditions(run) {
  ...
  addCheck(run, {
    id: 'post-display',
    layer: 'POST_CONDITION',
    status: linuxAlive ? 'FAIL' : 'PASS',
    code: linuxAlive ? 'POST_DISPLAY_STILL_ALIVE' : 'POST_DISPLAY_DEAD',
    component: `Xpra display :${session.display}`,
    cause: linuxAlive ? 'xpra info ainda responde após Stop.' : 'xpra info deixou de responder após Stop.',
    evidence: `display=:${session.display}`,
  });

  addCheck(run, {
    id: 'post-port',
    layer: 'POST_CONDITION',
    status: tcpOpen ? 'FAIL' : 'PASS',
    code: tcpOpen ? 'POST_PORT_STILL_OPEN' : 'POST_PORT_CLOSED',
    component: `127.0.0.1:${session.port}`,
    cause: tcpOpen ? 'A porta TCP do dry run continua aceitando conexão após Stop.' : 'A porta TCP do dry run deixou de aceitar conexão.',
    evidence: `port=${session.port}`,
  });
  ...
  addCheck(run, {
    id: 'post-websocket',
    layer: 'POST_CONDITION',
    status: ws.ok ? 'FAIL' : 'PASS',
    code: ws.ok ? 'POST_WEBSOCKET_STILL_OPEN' : 'POST_WEBSOCKET_CLOSED',
    component: 'Xpra WebSocket',
    cause: ws.ok ? 'Handshake WebSocket ainda abre após Stop.' : 'Handshake WebSocket não abre após Stop, como esperado.',
    evidence: ws.ok ? 'websocket=open' : `websocket=closed; probe=${ws.error || 'connection failed'}`,
    durationMs: ws.durationMs,
  });

  const ledger = readRuntimeLedger();
  const matching = ledger.filter(entry => entry.id === session.id || entry.port === session.port || entry.display === session.display);
  addCheck(run, {
    id: 'post-ledger',
    layer: 'POST_CONDITION',
    status: matching.length ? 'FAIL' : 'PASS',
    code: matching.length ? 'POST_LEDGER_NOT_CLEAN' : 'POST_LEDGER_CLEAN',
    component: 'POC1 runtime ledger',
    cause: matching.length ? 'O ledger contém referência conflitante após o dry run.' : 'Nenhuma referência do dry run ficou no ledger da POC1.',
    evidence: matching.length ? matching : `file=${RUNTIME_LEDGER_FILE}; matching=0`,
  });
...
async function writeArtifacts(run) {
  ...
  const telemetry = {
    ...
    metrics: run.metrics,
    proxy: run.session ? {
      httpRequests: run.session.metrics.proxyHttpRequests,
      webSocketConnections: run.session.metrics.proxyWebSocketConnections,
    } : null,
    checks: run.checks,
  };
  ...
  const lines = [
    ...
    `**Decision:** **${decisionFor(run) === 'GO' ? 'GO' : 'NO GO'}**`,
    `**xclock executed:** **NO**`,
    '',
    '## Boundaries',
    '',
...
async function validateStatic(run, requestedDistribution) {
  ...
  let distribution = normalizeName(requestedDistribution || snapshot.preferred || snapshot.default || '');
  ...
  const missingFlags = REQUIRED_XPRA_FLAGS.filter(flag => !cli.output.includes(flag));
  addCheck(run, {
  ...
  if (!displayScan.ok) {
    addCheck(run, {
      ...
      cause: 'Não foi possível inspecionar sockets/locks X11 na faixa reservada da POC1.', evidence: displayScan.error, durationMs: displayScan.durationMs,
    });
  } else {
    addCheck(run, {
      ...
      component: ':100..:149',
      cause: displayScan.occupied.length ? 'Existem displays ocupados na faixa, mas o preflight só usará um par comprovadamente livre.' : 'Nenhum socket/lock X11 foi encontrado na faixa reservada.',
  ...
  addCheck(run, {
    ...
    cause: portScan.free.length === 0 ? 'Nenhuma porta da faixa está disponível.' : portScan.occupied.length ? 'Algumas portas estão ocupadas; um par livre será selecionado.' : 'Toda a faixa de portas está livre.',
  ...
  if (orphanState.active.length) {
    addCheck(run, {
      id: 'orphans', layer: 'XPRA', status: 'FAIL', code: 'POC_RUNTIME_ACTIVE_DURING_PREFLIGHT', component: 'POC1 session manager',
      cause: 'Há sessão real da POC1 ativa; o preflight físico deve iniciar em estado limpo.', evidence: orphanState.active.map(session => ({ id: session.id, app: session.app, display: session.display, port: session.port })),
      durationMs: run.metrics.orphanScanMs,
    });
  } else if (orphanState.ledger.length) {
    addCheck(run, {
      id: 'orphans', layer: 'XPRA', status: 'FAIL', code: 'POC_LEDGER_NOT_CLEAN', component: 'POC1 runtime ledger',
      cause: 'O ledger contém sessões anteriores; nenhuma delas será encerrada automaticamente pelo preflight.', evidence: orphanState.orphaned,
      durationMs: run.metrics.orphanScanMs,
    });
  } else {
    addCheck(run, {
      id: 'orphans', layer: 'XPRA', status: 'PASS', code: 'POC_ORPHANS_CLEAR', component: 'POC1 runtime ledger',
      cause: 'Nenhuma sessão ativa ou entrada anterior foi encontrada no ledger.', evidence: `file=${RUNTIME_LEDGER_FILE}; entries=0`, durationMs: run.metrics.orphanScanMs,
    });
  }
  ...
  if (!run.pair) {
    ...
    addCheck(run, {
      ...
      cause: 'Par efêmero livre reservado logicamente para o dry run.', evidence: `display=:${run.pair.display}; port=${run.pair.port}`,
    });
  }
...
async function validateDryRunBoundaries(run, backendOrigin) {
  ...
  addCheck(run, {
    ...
    cause: tcp.ok ? 'Windows alcançou a porta publicada pelo Xpra dentro do WSL.' : 'Xpra está iniciado, mas a porta não é alcançável pelo backend Windows.',
  ...
  addCheck(run, {
    ...
    cause: directHttp.ok ? 'Cliente HTML5 respondeu diretamente na porta Xpra.' : 'TCP abriu, mas HTML5 não foi confirmado na resposta HTTP.',
    evidence: directHttp, durationMs: directHttp.durationMs,
  });
  ...
  addCheck(run, {
    ...
    cause: directWs.ok ? 'Handshake WebSocket direto com Xpra abriu.' : 'HTML5 respondeu, mas o handshake WebSocket direto falhou.',
  ...
  addCheck(run, {
    ...
    cause: proxyHttp.ok ? 'O backend CloudOS encaminhou o HTML5 pelo capability path.' : 'Xpra direto está disponível, mas o HTTP proxy CloudOS falhou.',
  ...
  addCheck(run, {
    ...
    cause: proxyWs.ok ? 'Handshake WebSocket atravessou o dispatcher/proxy CloudOS e alcançou Xpra.' : 'WebSocket direto funciona, mas o tunnel pelo CloudOS falhou.',
...
export async function startPhysicalPreflight({ ownerId, distribution, backendOrigin } = {}) {
  return queuePreflight(async () => {
    ...
    const existing = [...runs.values()].find(run => run.ownerId === normalizedOwner && run.phase === 'awaiting_iframe');
    ...
    const directoriesReady = prepareEvidenceDirectories(run);
    if (!directoriesReady) return finalizeEarly(run);
    ...
    addCheck(run, {
      ...
      cause: 'Backend, proxy e WebSocket passaram; falta o cliente CloudOS carregar o HTML5 e observar connection-established.', evidence: run.clientUrl,
    });
    await writeArtifacts(run);
    ...
    run.autoFinalizeTimer = setTimeout(() => {
      void finalizePhysicalPreflight({
        ...
        iframe: {
          status: 'FAIL',
          code: 'IFRAME_VALIDATION_TIMEOUT',
          cause: `O frontend não confirmou a conexão do iframe em ${AUTO_FINALIZE_MS}ms.`,
          evidence: 'automatic-finalize',
        },
      }).catch(() => undefined);
    }, AUTO_FINALIZE_MS);
...
export async function finalizePhysicalPreflight({ runId, ownerId, iframe = {} } = {}) {
  return queuePreflight(async () => {
    ...
    if (!run) {
      const error = new Error('Execução de preflight não encontrada ou já expirada.');
    ...
    addCheck(run, {
      ...
      cause: iframeStatus === 'PASS'
        ? 'O iframe CloudOS carregou o HTML5 e recebeu connection-established sem iniciar xclock.'
        : String(iframe.cause || 'O iframe não confirmou conexão ao Xpra.'),
      evidence: iframeStatus === 'PASS'
        ? `loadMs=${Number(iframe.loadMs) || 0}; connectionMs=${Number(iframe.connectionMs) || 0}`
        : String(iframe.evidence || 'frontend iframe validation failed'),
...
export async function shutdownPhysicalPreflight() {
  for (const run of runs.values()) {
    ...
    if (run.autoFinalizeTimer) clearTimeout(run.autoFinalizeTimer);
    addCheck(run, {
      id: 'shutdown-interrupt', layer: 'POST_CONDITION', status: 'FAIL', code: 'PREFLIGHT_INTERRUPTED_BY_BACKEND_SHUTDOWN', component: 'CloudOS backend lifecycle',
      cause: 'O backend encerrou enquanto o dry run estava ativo.', evidence: 'shutdownPhysicalPreflight',
    });
    await stopDryRun(run).catch(() => undefined);
    await validatePostConditions(run).catch(() => undefined);
    run.phase = 'complete';
