...
func handleFilesRequest(method string, params json.RawMessage, writer *protocol.SecureChannel, id string) (bool, error) {
	...
	case "fs.mkdir":
		...
		if err := manager.Mkdir(request.Path, request.Mode); err != nil {
			return fail(err)
		}
	...
	case "fs.rename", "fs.move":
		...
		if err := manager.Rename(request.Source, request.Destination); err != nil {
			return fail(err)
		}
	...
	case "fs.trash.delete":
		...
		if err := manager.DeleteTrash(request.ID); err != nil {
			return fail(err)
		}
