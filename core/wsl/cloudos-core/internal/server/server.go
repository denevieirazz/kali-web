...
func Run(stdin io.Reader, stdout, stderr io.Writer) error {
	...
	srv := &Server{secret: secret, distro: distro}
...
func (s *Server) handleRequest(owner string, manager *processcore.Manager, writer *protocol.SecureChannel, env protocol.Envelope) (bool, error) {
	...
	case "session.create":
		var options processcore.CreateOptions
	...
	case "terminal.create":
		...
		return false, writeOK(writer, env.ID, status)
	case "session.input":
		var input struct{ SessionID, Data string }
		...
		if err := manager.Input(owner, input.SessionID, data); err != nil {
			return false, err
		}
	...
	case "session.resize":
		...
		if json.Unmarshal(request.Params, &resize) != nil {
			return false, coded("REQUEST_INVALID")
		}
		if err := manager.Resize(owner, resize.SessionID, resize.Rows, resize.Cols); err != nil {
			return false, err
		}
		return false, writeOK(writer, env.ID, map[string]bool{"accepted": true})
	case "session.signal":
		...
		if json.Unmarshal(request.Params, &signal) != nil {
			return false, coded("REQUEST_INVALID")
		}
		if err := manager.Signal(owner, signal.SessionID, signal.Signal); err != nil {
			return false, err
		}
		return false, writeOK(writer, env.ID, map[string]bool{"accepted": true})
	case "session.status":
		...
		if json.Unmarshal(request.Params, &statusRequest) != nil {
			return false, coded("REQUEST_INVALID")
		}
		status, err := manager.Status(owner, statusRequest.SessionID)
		if err != nil {
			return false, err
		}
		return false, writeOK(writer, env.ID, status)
	case "session.wait":
		...
		if json.Unmarshal(request.Params, &wait) != nil {
			return false, coded("REQUEST_INVALID")
		}
		status, err := manager.Wait(owner, wait.SessionID, time.Duration(wait.TimeoutMs)*time.Millisecond)
		if err != nil {
			return false, err
		}
		return false, writeOK(writer, env.ID, status)
...
func safeMessage(code string) string {
	messages := map[string]string{
		"REQUEST_INVALID": "request is invalid", "METHOD_NOT_FOUND": "method is not supported",
		"EXECUTABLE_DENIED": "executable is not allowed", "ARGUMENT_LIMIT": "argument limit exceeded",
